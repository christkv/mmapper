...
var Schema = module.exports = function(schema_name, specification) {
  ...
  var schema = function(field, transient) {
    return {
      of: function(type) {
        ...
        if(type.chain) {
          schema_rules.fields[field] = un_chain_types(type);
          if(transient) schema_rules.fields[field].transient = true;
          return;
        }
  ...
  schema.transform = {
    before: {
      create: function(func) {
        schema_rules.transform.before.create.unshift({func: func});
      }, 

      update: function(func) {
        schema_rules.transform.before.update.unshift({func: func});
      }, 

      remove: function(func) {
        schema_rules.transform.before.remove.unshift({func: func});
      }, 
    }
  }  
  ...
  schema.validate = {
    ...
    before: {
      create: function(func) {
        schema_rules.validate.before.create.push({func: func});
      },

      update: function(func) {
        schema_rules.validate.before.update.push({func: func});
      },
    ...
    async: {
      before: {
        create: function(func) {
          schema_rules.validate.before.create.push({func: func, async: true});
        },

        update: function(func) {
          schema_rules.validate.before.update.push({func: func, async: true});
        },
...
var add_transform_methods = function(schema_rules) {
  // console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ add_transform_methods")
  // console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ add_transform_methods")
  // console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ add_transform_methods")
  ...
  for(var i = 0; i < keys.length; i++) {
    ...
    if(field.spec && field.spec.transform.before.create) {
      var funcs = schema_rules.transform.before.create;
      var field_funcs = field.spec.transform.before.create;
      
      // Add all the functions
      for(var j = 0; j < field_funcs.length; j++) {
        schema_rules.transform.before.create.push({func: field_funcs[j], field: field, name: keys[i]});
      }

      // schema_rules.transform.before.create = funcs.concat(field_funcs);
    } else if(field.spec && field.spec.transform.before.update) {
      var funcs = schema_rules.transform.before.update;
      var field_funcs = field.spec.transform.before.update;
      schema_rules.transform.before.update = funcs.concat(field_funcs);
    } else if(field.spec && field.spec.transform.before.remove) {
      var funcs = schema_rules.transform.before.remove;
      var field_funcs = field.spec.transform.before.remove;
      schema_rules.transform.before.remove = funcs.concat(field_funcs);
    }
  }  
}
